...
- _RPCOnionEnabled_ is only available as command line switch (start up parameter) and environment variable, so it is not in the config file.
...
To start Wasabi with _rpconionservice_ enabled:

```bash
wassabeed --rpconionservice=true
```
